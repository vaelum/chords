...
const { useState: useStateSV, useEffect: useEffectSV, useRef: useRefSV, useMemo: useMemoSV, useCallback: useCallbackSV } = React;
...
function SongView({ song, playlist, store, onBack,
  ...
  async function removeFromPlaylist() {
    ...
    try {
      ...
      toast({ title: "Couldn't remove", desc: e.message || String(e), tone: 'destructive' });
    }
  }
  ...
  useEffectSV(() => {
    ...
    if (!el) { setChordsOverflow(false); return; }
  ...
  const lineWeights = useMemoSV(
    () => parsedLines.map(l =>
      1 + DENSITY_STRENGTH * (l.tokens ? l.tokens.reduce((a, t) => a + (t.chord ? 1 : 0), 0) : 0)),
    [parsedLines]
  );
  const avgWeight = useMemoSV(
    () => lineWeights.length ? lineWeights.reduce((a, w) => a + w, 0) / lineWeights.length : 1,
    [lineWeights]
  );
  ...
  useEffectSV(() => { densityModeRef.current = densityMode; }, [densityMode]);
  function setDensity(on) {
    setDensityMode(on);
    try { localStorage.setItem(SCROLL_DENSITY_KEY, on ? '1' : '0'); } catch (e) {}
  }
  ...
  function handleTranspose(delta) {
    ...
    if (readOnly) { setLocKey(newKey); setLocBody(newBody); }
    else store.updateSong(song.id, { key: newKey, body: newBody });
  }

  function handleCapo(delta) {
    const newCapo = Math.min(11, Math.max(0, vCapo + delta));
    if (readOnly) setLocCapo(newCapo);
    else store.updateSong(song.id, { capo: newCapo });
  }

  function handleTempo(delta) {
    const newTempo = Math.min(300, Math.max(20, vTempo + delta));
    if (readOnly) setLocTempo(newTempo);
    else store.updateSong(song.id, { tempo: newTempo });
  }
  ...
  function handleSpeed(delta) {
    ...
    if (!readOnly) store.updateSong(song.id, { scrollSpeed: newSpeed });
  }
  ...
  function jumpToSection(domIndex) {
    ...
    if (!container) { setSectionsOpen(false); return; }
  ...
  return (
    <div className="sv-shell" data-chord-color={chordColor}>
      ...
      {!autoscroll && (
        <div className="sv-header">
          ...
          <div style={{ minWidth: 0 }}>
            <div className="sv-title">{song.title}</div>
      ...
      {/* Same actions as the direct buttons, for the collapsed (narrow) layout.
          Delete is intentionally absent here — it lives only in the library and
          playlist views. */}
      {!readOnly && (
        <Menu open={!!menuEl} anchor={menuEl} onClose={() => setMenuEl(null)} items={[
          ...
          ...(playlist ? [
            ...
            { sep: true },
            { label: 'Remove from playlist', icon: 'trash', destructive: true, onSelect: removeFromPlaylist },
          ] : []),
        ]} />
      )}

      {!autoscroll && playlist && (
        <div className="sv-playlist-banner">
          ...
          {playlist.shared && (
            <>
              ...
              <div className="collab-avatars">
                {collabs.slice(0, 4).map(u => <span key={u.id} className={`av ${u.color}`} style={{ width: 20, height: 20, fontSize: 9 }}>{u.initials}</span>)}
              </div>
              <span style={{ color: 'var(--muted-foreground)' }}>Edits sync to {collabs.length} collaborators</span>
            </>
          )}
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
            <Btn variant="outline" size="sm" disabled={!onPrev} onClick={() => onPrev && onPrev()}>
              <Icon name="back" size={14} /> Prev
            </Btn>
            <Btn variant="outline" size="sm" disabled={!onNext} onClick={() => onNext && onNext()}>
      ...
      {!autoscroll && (
        <div className="sv-meta-bar">
          <button className={`meta-pop-btn${keyPopOpen ? ' on' : ''}`}
                  onClick={() => { setKeyPopOpen(v => !v); setCapoPopOpen(false); setTempoPopOpen(false); }}>
            <span className="label">Key</span>
            <span className="val">{vKey}</span>
          </button>

          <button className={`meta-pop-btn${capoPopOpen ? ' on' : ''}`}
                  onClick={() => { setCapoPopOpen(v => !v); setKeyPopOpen(false); setTempoPopOpen(false); }}>
            <span className="label">Capo</span>
            <span className="val">{vCapo}</span>
          </button>

          <button className={`meta-pop-btn${tempoPopOpen ? ' on' : ''}`}
                  onClick={() => { setTempoPopOpen(v => !v); setKeyPopOpen(false); setCapoPopOpen(false); }}>
            <span className="label">BPM</span>
            <span className="val">{vTempo}</span>
          </button>
      ...
      {/* Key popup — screen-centred */}
      {keyPopOpen && (
        <>
          <div style={{ position: 'fixed', inset: 0, zIndex: 49 }}
               onClick={() => setKeyPopOpen(false)} />
          <div className="sv-center-popup">
            <div className="sv-center-popup-head">
              <span>Transpose key</span>
              <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setKeyPopOpen(false)}>
                <Icon name="close" size={14} />
              </button>
            </div>
            <div className="meta-stepper-row">
              <button className="as-icon-btn" onClick={() => handleTranspose(-1)} aria-label="Transpose down">
                <Icon name="minus" size={16} />
              </button>
              <span className="meta-stepper-val">{vKey}</span>
              <button className="as-icon-btn" onClick={() => handleTranspose(1)} aria-label="Transpose up">
      ...
      {capoPopOpen && (
        <>
          ...
          <div className="sv-center-popup">
            <div className="sv-center-popup-head">
              <span>Capo fret</span>
              <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setCapoPopOpen(false)}>
                <Icon name="close" size={14} />
              </button>
            </div>
            <div className="meta-stepper-row">
              <button className="as-icon-btn" onClick={() => handleCapo(-1)} aria-label="Capo down" disabled={!vCapo}>
                <Icon name="minus" size={16} />
              </button>
              <span className="meta-stepper-val">{vCapo}</span>
              <button className="as-icon-btn" onClick={() => handleCapo(1)} aria-label="Capo up">
                <Icon name="plus" size={16} />
              </button>
            </div>
          </div>
      ...
      {tempoPopOpen && (
        <>
          ...
          <div className="sv-center-popup">
            <div className="sv-center-popup-head">
              <span>Tempo</span>
              <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setTempoPopOpen(false)}>
                <Icon name="close" size={14} />
              </button>
            </div>
            <div className="meta-stepper-row">
              <button className="as-icon-btn" onClick={() => handleTempo(-5)} aria-label="Tempo down">
                <Icon name="minus" size={16} />
              </button>
              <span className="meta-stepper-val">{vTempo}</span>
              <button className="as-icon-btn" onClick={() => handleTempo(5)} aria-label="Tempo up">
                <Icon name="plus" size={16} />
              </button>
            </div>
          </div>
      ...
      {/* Chords bar — own row, tappable, shows overflow indicator */}
      {!autoscroll && chords.length > 0 && (
      ...
      {/* Chords popup — screen-centred */}
      {chordsOpen && (
        <>
          <div style={{ position: 'fixed', inset: 0, zIndex: 49 }}
               onClick={() => setChordsOpen(false)} />
          <div className="sv-center-popup" style={{ minWidth: 260 }}>
            <div className="sv-center-popup-head">
              <span>Chords used</span>
              <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setChordsOpen(false)}>
                <Icon name="close" size={14} />
              </button>
            </div>
