...
def server_deploy(ctx, args, inner):
    """Compile the frontend, ship the key, deploy, ship the snapshot off-box,
    then print the admin login.
    ...
    rc = inner()
    if rc == 0:
        _ship_backup(ctx, remote)
        _print_admin_login(ctx, remote)
    return rc


# --------------------------------------------------------------------------- #
# Shipping the deploy snapshot off the box
# --------------------------------------------------------------------------- #
# The harness takes a pre-deploy snapshot of the data directory and leaves it in
# backup_dir on the remote. A backup that only exists on the machine it is
# protecting is half a backup, so it is pushed to the same object storage the
# nightly job uses and the local copy is dropped. `chords-backup:` is an rclone
# alias in the remote's rclone config, so the bucket is named only on the host.
#
# Ordering matters: copy, VERIFY, and only then delete. Never the other way
# round — a failed upload followed by a delete loses the restore point the
# deploy just created.
_SHIP_SCRIPT = r"""
set -e
dir=~/.chords-backup
latest=$(ls -1t "$dir"/chords-*.zip 2>/dev/null | head -1)
if [ -z "$latest" ]; then echo "no snapshot to ship"; exit 0; fi
command -v rclone >/dev/null || { echo "rclone is not installed on the remote"; exit 1; }
rclone copy "$latest" chords-backup: --s3-no-check-bucket
rclone check "$latest" chords-backup: --size-only --one-way >/dev/null
rm -f "$latest"
echo "shipped $(basename "$latest") -> chords/ (local copy removed)"
"""


def _ship_backup(ctx, remote):
    """Push the snapshot the harness just took to object storage."""
    if ctx.dry_run:
        return
    r = proc.capture(["ssh", remote, _SHIP_SCRIPT])
    if r.ok:
        last = r.out.strip().splitlines()[-1] if r.out.strip() else "done"
        ui.ok("backup", last)
    else:
        # Not fatal: the deploy itself succeeded and the snapshot is still on
        # the remote. Say so loudly rather than failing the deploy.
        ui.warn("backup NOT shipped:", (r.err or r.out).strip()[:200])
        ui.plain("      The snapshot is still in ~/.chords-backup on the remote.")
